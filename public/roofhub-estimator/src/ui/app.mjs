...
export class RoofHubEstimator {
  ...
  render(focus=false){
    ...
    if(!this.cssLink){this.cssLink=document.createElement('link');this.cssLink.rel='stylesheet';this.cssLink.href=css;this.root.append(this.cssLink);}if(!this.stage){this.stage=document.createElement('div');this.root.append(this.stage);}this.stage.innerHTML=`<div class="rh-shell"><div class="rh-toolbar"><div class="rh-breadcrumb"><a href="/">RoofHub</a> ${icon('chevron')} <a href="/tools">Tools</a> ${icon('chevron')} Detailed estimator</div><div class="rh-actions"><button class="rh-tool-link" data-action="import-project">${icon('upload')} Open project</button><button class="rh-tool-link" data-action="export-project">${icon('download')} Save progress</button><button class="rh-tool-link" data-action="restart">Start over</button></div></div>
